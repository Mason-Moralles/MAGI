# Единый README по тестированию MAGI
...
| Описание тестов | Реализация тестов | Основные доказательства |
|---|---|---|
| Карта требований и проверок | Требования разложены по веткам UI, API, БД и надёжности | [MAGI-MindMap.svg](../MAGI-MindMap.svg) |
| Автоматизация API | Подготовлены Postman collection и Newman-прогон для ключевых API-сценариев | [MAGI_API.postman_collection.json](../tests/postman/MAGI_API.postman_collection.json), [MAGI_API.newman-report.json](../tests/postman/reports/MAGI_API.newman-report.json) |
| Проверка БД | Есть SQL-файл для ручной валидации SQLite после API- и UI-сценариев | [kt4_db_validation.sql](../tests/sql/kt4_db_validation.sql) |
| UI-автотесты | Реализован отдельный WPF UI test suite на Appium + WinAppDriver с Page Object Model | [MAGI.AdminPanel.UiTests.csproj](../AdmPanel/MAGI.AdminPanel.UiTests/MAGI.AdminPanel.UiTests.csproj) |
| Сквозной сценарий API + UI + DB | Реализован интеграционный тест, создающий данные через API, изменяющий их через UI и подтверждающий результат в SQLite | [IntegratedApiUiDbTests.cs](../AdmPanel/MAGI.AdminPanel.UiTests/Tests/IntegratedApiUiDbTests.cs) |
| Итоговый отчёт по тестированию | В одном документе собраны покрытие, результаты, артефакты и вывод по готовности проекта | Этот README целиком |
...
![Карта требований и проверок](images/kt2/MAGI-MindMap.png)
...
![Запуск Postman/Newman](images/kt3/kt3-1.png)

![Результат прогона API-тестов](images/kt3/kt3-2.png)

По ранее зафиксированным результатам коллекция была успешно выполнена: `9 requests`, `18 assertions`, `0 failures`.

---

## 6. SQL-проверки состояния БД
...
Проверка выполняется без SQLite IDE напрямую из PowerShell, что позволяет одновременно показать команду запуска и фактический результат SQL-валидации.
...
![Запуск SQL-валидации](images/kt4/kt4-1.png)

![Результат SQL-проверок](images/kt4/kt4-2.png)

Результат тестов подтверждается не только ответами API или поведением UI, но и фактическим состоянием БД.

---

## 7. Автотесты WPF-интерфейса
...
![Результат запуска UI-автотестов](images/kt5/kt5.png)

---

## 8. Интеграционный сценарий API + UI + DB
...
## 9. Итоговый отчёт по тестированию
...
В итоге, пункт номер 6 из задания на дипломный проект "6. Реализовать и протестировать взаимодействие между модулями системы;" Закрывается следующими пунками:

- Формализованным планом тестирования;
- Картой требований и покрытия;
- Автоматизированной проверкой API через Postman/Newman;
- SQL-проверками реального состояния SQLite;
- Отдельным набором UI-автотестов для WPF AdminPanel;
- Интеграционным сценарием API + UI + DB;
- Итоговым сводным описанием результатов, артефактов и готовности проекта к защите.